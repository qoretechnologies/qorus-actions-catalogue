import { DedupeStrategy, Polling, pollingHelper } from 'core/common';
import { createTrigger, TriggerStrategy, OAuth2PropertyValue } from 'core/framework';
...
const polling: Polling<OAuth2PropertyValue, { database_id: string | undefined }> = {
  ...
  items: async ({ auth, propsValue, lastItemId }) => {
    ...
    const items = await getResponse(auth, propsValue.database_id!, lastUpdatedDate);
