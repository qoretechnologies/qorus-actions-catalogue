...
import { IDealInterface } from 'hubspot/models/deals';

// Defining a function to fetch deal by id
...
const getDeal = async ({ id }: TActionData<typeof options>) => {
  try {
    const data: IDealInterface = await hubspotRequest(`/objects/deals/${id}`, 'GET');
    return data;
  } catch (error) {
