...
// Defining a function to get a contacts
const options: IActionOptions = null;
export const response_type: IActionResponse = {
  results: {
    type: 'list',
    name: 'results',
    display_name: L.en.apps.hubspot.actions.contacts.results.displayName(),
    short_desc: L.en.apps.hubspot.actions.contacts.results.shortDesc(),
    desc: L.en.apps.hubspot.actions.contacts.results.longDesc(),
    example_value: [],
  },
  id: {
    type: 'number',
...
const getContacts = async () => {
  try {
    const data: TActionData<typeof options> = await hubspotRequest(`/objects/contacts`, 'GET');
    return data;
  } catch (error) {
