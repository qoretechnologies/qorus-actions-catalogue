...
export class Processors {
  ...
  static string: ProcessorFn<string | number | undefined | null, string | null | undefined> = (property, value) => {
    if (isNil(value)) {
      return value;
    }
    if (typeof value === 'object') {
    ...
    try {
      ...
      if (isBase64(urlOrBase64, { allowMime: true })) {
        ...
        if (matches && matches?.length === 3) {
          ...
          return new ApFile(filename + '.' + extension, Buffer.from(base64, 'base64'), extension);
        }
      }
