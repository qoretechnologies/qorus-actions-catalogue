...
import { Action, ActionType, BranchAction, LoopOnItemsAction, SingleActionSchema } from './actions/action';
import {
  AddActionRequest,
  DeleteActionRequest,
...
async function transferStepAsync<T extends Step>(step: Step, transferFunction: (step: T) => Promise<T>): Promise<Step> {
...
function moveAction(flowVersion: FlowVersion, request: MoveActionRequest): FlowVersion {
  ...
  if (clonedSourceStep.type === ActionType.LOOP_ON_ITEMS || clonedSourceStep.type === ActionType.BRANCH) {
